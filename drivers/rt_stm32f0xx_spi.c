...
rt_inline uint16_t get_spi_BaudRatePrescaler(rt_uint32_t max_hz)
{
    ...
    if(max_hz >= SystemCoreClock/2 && SystemCoreClock/2 <= 24000000) {
...
static rt_err_t configure(struct rt_spi_device* device, struct rt_spi_configuration* configuration)
{
    struct stm32f0xx_spi_bus* stm32f0xx_spi_bus = (struct stm32f0xx_spi_bus*)device->bus;
    ...
    /* init SPI */
    SPI_I2S_DeInit(stm32f0xx_spi_bus->SPI);
    SPI_Init(stm32f0xx_spi_bus->SPI, &SPI_InitStructure);
    /* Enable SPI_MASTER */
    SPI_Cmd(stm32f0xx_spi_bus->SPI, ENABLE);
    SPI_CalculateCRC(stm32f0xx_spi_bus->SPI, DISABLE);
...
static rt_uint32_t xfer(struct rt_spi_device* device, struct rt_spi_message* message)
{
    ...
    SPI_TypeDef* SPI = stm32f0xx_spi_bus->SPI;
    struct stm32f0xx_spi_cs* stm32f0xx_spi_cs = (struct stm32f0xx_spi_cs*)device->parent.user_data;
    rt_uint32_t size = message->length;

    /* take CS */
    if(message->cs_take) {
        GPIO_ResetBits(stm32f0xx_spi_cs->GPIOx, stm32f0xx_spi_cs->GPIO_Pin);
    }
